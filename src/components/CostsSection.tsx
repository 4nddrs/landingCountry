    ...
    <section className="min-h-screen flex items-center justify-center py-20 px-6 bg-gradient-to-b from-[#0f1629] to-[#1a2333]">
      <div className="max-w-6xl mx-auto w-full">
                          ...
                          return (
                            <motion.tr
                              ...
                              className={`
                                ...
                                ${cost.isSubtotal ? 'bg-blue-600/10 border-y-2 border-blue-500/30' : 'border-b border-blue-500/10'}
                                ${cost.isTotal ? 'bg-gradient-to-r from-blue-600/25 via-blue-500/15 to-blue-600/25 border-y-2 border-blue-400/40 shadow-lg shadow-blue-500/10' : ''}
                              ...
                              `}
                            >
                              <td className={`px-10 py-8 ${cost.isTotal || cost.isSubtotal ? 'font-bold text-white text-xl' : 'text-slate-300 text-base font-light'} group-hover:text-white transition-colors`}>
                                {cost.categoria}
                              </td>
                              <td className={`px-10 py-8 text-right font-mono tabular-nums tracking-wider ${cost.isTotal ? 'font-bold text-4xl text-blue-400 drop-shadow-[0_0_15px_rgba(96,165,250,0.5)]' : cost.isSubtotal ? 'font-bold text-2xl text-white' : 'text-slate-300 text-xl font-medium'}`}>
                                {cost.isTotal || cost.isSubtotal ? (
                                  <span className="inline-flex items-center gap-3">
                                    {cost.subtotal}
                                    {cost.isTotal && (
                                      <span className="inline-block w-2.5 h-2.5 rounded-full bg-blue-400 animate-pulse shadow-lg shadow-blue-400/50" />
                                    )}
                                  </span>
                                ) : (
                                  cost.subtotal
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12">
          {[
            { label: "Hardware", value: "1.7%", color: "from-blue-600/10" },
            { label: "Software", value: "5.9%", color: "from-purple-600/10" },
            { label: "Desarrollo", value: "83.3%", color: "from-blue-600/20" }
          ].map((stat, index) => (
            <motion.div
              key={index}
              ...
              className={`p-6 rounded-2xl bg-gradient-to-br ${stat.color} to-transparent border border-blue-500/10 text-center`}
            >
